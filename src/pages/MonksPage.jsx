                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5 pb-24">
                  ...
                  {tab === "movement" && (
                    <div className="space-y-4">
                      ...
                      {/* Chaturmas History List */}
                      <div className="border p-3.5 rounded-xl bg-slate-50 space-y-3">
                        <div className="flex justify-between items-center border-b pb-1">
                          <span className="text-xs font-bold text-slate-700">🍁 Chaturmas History (Year-Wise)</span>
                          <Button type="button" size="sm" variant="outline" onClick={addChaturmasHistory} className="h-6 text-[10px] font-bold">
                            + Add Chaturmas Year
                          </Button>
                        </div>
                        {(form.chaturmasHistory || []).map((c, idx) => (
                          <div key={idx} className="border p-3 rounded-lg bg-white space-y-2.5 relative shadow-sm">
                            <button type="button" onClick={() => removeChaturmasHistory(idx)} className="absolute top-2 right-2 text-slate-400 hover:text-red-500">
                              <X className="h-4 w-4" />
                            </button>
                            <div className="grid grid-cols-3 gap-2">
                              <div>
                                <Label className="text-[10px] font-bold">Year</Label>
                                <Input className="h-8 mt-0.5" value={c.year} onChange={(e) => updateChaturmasHistory(idx, "year", e.target.value)} placeholder="e.g. 2026" />
                              </div>
                              <div>
                                <Label className="text-[10px] font-bold">City</Label>
                                <Input className="h-8 mt-0.5" value={c.city} onChange={(e) => updateChaturmasHistory(idx, "city", e.target.value)} />
                              </div>
                              <div>
                                <Label className="text-[10px] font-bold">State</Label>
                                <Input className="h-8 mt-0.5" value={c.state} onChange={(e) => updateChaturmasHistory(idx, "state", e.target.value)} />
                              </div>
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                              <div>
                                <Label className="text-[10px] font-bold">Linked Temple / Upashray</Label>
                                <select className="w-full mt-0.5 h-8 rounded border bg-white px-2 text-xs focus:outline-none"
                                  value={c.orgId} onChange={(e) => updateChaturmasHistory(idx, "orgId", e.target.value)}>
                                  <option value="">Select Temple...</option>
                              ...
                              <div>
                                ...
                                <select className="w-full mt-0.5 h-8 rounded border bg-white px-2 text-xs focus:outline-none"
                                  value={c.status} onChange={(e) => updateChaturmasHistory(idx, "status", e.target.value)}>
                                  <option value="Completed">Completed</option>
                                  <option value="Ongoing">Ongoing</option>
